...
\title{Mapping Automata to zk-SNARKS \\ Using Category Theory}
\author{
...
\def\titlerunning{Mapping Automata to zk-SNARKS}
...
\begin{abstract}
  ...
  automata state spaces into boolean circuits, leveraging on the fact that 
...
Smart contracts, especially if hosted on blockchain~[], are immutable
...
that are reliable, easy to analyze and correct-by-construction. These requirements
 ...
 automata~[] are considered easy to implement, very structured, 
 and make possible to prove roperties of the computations 
 being performed. 
...
syncing from there. This ``trust Vs. feasibility'' issue can be resolved by recursive 
zk-SNARKS~[], in which snarks are used to verify, using a few Kbs, that the current 
...
cryptography, providing a categorical way to turn automata into zk-SNARKs 
that verify how a sequence of inputs leading to a state change follows the 
rules specified by the automaton itself. To do this, we bypass the problem 
...
explain the links between automata and free categories. 
In Section~\ref{sec: turning executions into circuits} 
we show how to turn a given sequence of state changes 
for a given automaton into a boolean circuit. We then obtain a 
...
circuits which accept the specification defining an automaton 
...
in $\Bkp$, as it becomes evident by drawing it:
%
%
\begin{equation*}
  \begin{tikzpicture}
...
$\Snark{\_}$ cannot be made improved to a 
...
zero knowledge proof that any specified path in the graph is valid or not.
%
%
%
...
of the state space of an automaton can reveal a lot about 
what the automaton is used for. We would like to keep this 
information private.
...
if we given different graphs specifications in the composition?

The point is that notice that the categorical structure 
...
\begin{definition}\label{def: definition of bzkp}
  Fix a natural number $k$. We denote with $\Bzkp{k}$ the \emph{bicategory of zero knowledge proof circuits of size $k$} defined 
  as follows:
  %
  %
  \begin{itemize}
